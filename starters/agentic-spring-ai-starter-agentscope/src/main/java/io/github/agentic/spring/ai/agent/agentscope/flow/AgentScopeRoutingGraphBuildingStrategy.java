 ...
 */
package io.github.agentic.spring.ai.agent.agentscope.flow;
...
import io.github.agentic.spring.ai.graph.action.AsyncMultiCommandAction;
...
import io.github.agentic.spring.ai.graph.agent.flow.node.RoutingMergeNode;
import io.github.agentic.spring.ai.graph.agent.flow.node.TransparentNode;
import io.github.agentic.spring.ai.graph.agent.flow.strategy.AbstractFlowGraphBuildingStrategy;
import io.github.agentic.spring.ai.graph.agent.flow.strategy.FlowGraphBuildingStrategy;
 ...
 * io.github.agentic.spring.ai.graph.agent.flow.strategy.RoutingGraphBuildingStrategy} (root →
...
public class AgentScopeRoutingGraphBuildingStrategy extends AbstractFlowGraphBuildingStrategy {
	...
	/** Strategy type for registration with {@link io.github.agentic.spring.ai.graph.agent.flow.strategy.FlowGraphBuildingStrategyRegistry}. */
	...
	protected void buildCoreGraph(FlowGraphBuilder.FlowGraphConfig config) throws GraphStateException {
		...
		for (Agent subAgent : config.getSubAgents()) {
			io.github.agentic.spring.ai.graph.agent.flow.strategy.FlowGraphBuildingStrategy.addSubAgentNode(subAgent, graph);
	...
	public KeyStrategyFactory generateKeyStrategyFactory(FlowGraphBuilder.FlowGraphConfig config) {
		...
		return () -> {
			Map<String, io.github.agentic.spring.ai.graph.KeyStrategy> strategies = new HashMap<>(parent.apply());
